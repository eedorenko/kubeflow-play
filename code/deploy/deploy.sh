...
az login --service-principal --username $AZ_CLIENT_ID --password $AZ_CLIENT_SECRET -t $AZ_TENANT_ID
...
model_id=$(az ml model list --model-name $MODEL --workspace-name $WORKSPACE -g $RESOURCE_GROUP --subscription-id $AZ_SUBSCRIPTION_ID --tag run_id=$RUN_ID | jq -r '.[0] | .id') 
echo $model_id
